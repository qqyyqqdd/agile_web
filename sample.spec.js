describe('add todo', function () {
    ...
    it('should items amount correct',async function() {
    	let todoList = await page.waitFor('#todo-list');
    	let expectItemAmount = await page.evaluate(()=>{
    		var list = document.getElementById('todo-list');
    		return list.getElementsByTagName('li').length;
    	})
    	
    	expect(expectItemAmount).to.eql(4);
    })
    it('should finish item correct',async function(){
    	let todoList = await page.waitFor('#todo-list');
    	await page.click('.toggle', {delay: 500});
    	let finishItem = await page.evaluate(todoList => todoList.firstChild.querySelector('input').checked, todoList);    	
    	expect(finishItem).to.eql(false);
    })
  });